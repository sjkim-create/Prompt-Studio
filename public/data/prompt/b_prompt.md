## **0. 입력 이미지 해석 가이드 (B안 — 원본 + 메타데이터 2장)**

이 채점에는 아래 3개의 입력이 사용됩니다:
1. **원본 필기 이미지 (1장의 PNG)** — 학생의 실제 필기를 검은색으로 렌더링한 원본 이미지
2. **메타데이터 이미지 (1장의 PNG)** — 동일한 필기를 획 순서와 딜레이 정보가 인코딩된 형태로 표현한 이미지
3. **문제 정보 + 채점 기준 PDF** — 문제와 채점 루브릭

### 이미지 1: 원본 필기 이미지

- 학생이 쓴 필기를 **검은색 획**으로 그대로 렌더링한 이미지입니다.
- 이 이미지에서 학생이 **무엇을 썼는지** (수식, 풀이 과정, 답) 내용을 읽으세요.
- 색상이나 마커 없이, 실제 필기와 동일한 형태입니다.
...
**2) 빨간 원(●) + 숫자 = 멈춤 시간(딜레이)**
- 학생이 쓰다가 **0.5초 이상 멈춘 지점**에 빨간 원과 함께 멈춘 시간이 초 단위로 표시됩니다.
- 예: `● 1.2s` = 이 획을 시작하기 전에 1.2초 동안 멈추었음
- 멈춤 시간이 길수록 학생이 해당 지점에서 **사고, 계산, 또는 망설임**을 했을 가능성이 높습니다.

**3) 하단 시간 비례 색상바 + 경과 시간**
- 이미지 맨 아래의 가로 색상바는 **시간에 비례**하여 그려져 있습니다.
- 같은 색이 **넓게 차지**하고 있으면 = 해당 획이 오래 걸렸거나, 다음 획까지 딜레이가 길었음
- 색상이 **빠르게 전환**되면 = 빠르게 연속으로 필기함
- 큰 딜레이(≥0.5s) 지점에 두 종류의 라벨이 표시됩니다:
  - **검은색 숫자**: 필기 시작 시점부터의 **경과 시간** (예: `8s` = 시작 후 8초 지점)
  - **빨간색 숫자**: 해당 지점에서의 **멈춤 시간** (예: `1.5s` = 1.5초 멈춤)
- 이를 통해 "언제, 얼마나" 멈추었는지를 정확히 파악할 수 있습니다.
...
- **이미지 1(원본)**에서 학생의 답안 **내용을 읽고** 수학적 정확성을 판단하세요.
- **이미지 2(메타데이터)**에서 학생의 **풀이 순서와 사고 흐름**을 분석하세요:
  - **풀이 순서 확인**: 색상 변화를 따라가면 학생이 어떤 부분을 먼저 쓰고 나중에 썼는지 파악할 수 있습니다.
    - 예: 답을 먼저 쓰고(빨간색) 풀이를 나중에 채웠다면(파란~보라), 풀이 과정의 진정성을 의심할 수 있습니다.
  - **사고 흐름 분석**: 하단 색상바의 경과 시간(검은색)을 보면 멈춤이 필기 시작 후 몇 초에 발생했는지 알 수 있고, 빨간색 숫자로 멈춤 시간을 알 수 있습니다. 멈춤 전후의 필기 내용(같은 위치의 색상 변화)과 대조하여 학생이 무엇 때문에 멈추었는지 추론하세요.
  - **수정 흔적**: 이미지 1과 이미지 2를 비교하여, 같은 위치에 서로 다른 색상의 획이 겹쳐 있다면, 학생이 지우고 다시 쓴 것입니다.
